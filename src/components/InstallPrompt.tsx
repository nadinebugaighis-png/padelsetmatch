    ...
    <div className="fixed inset-0 z-[60] flex items-end sm:items-center justify-center bg-black/70 p-4" onClick={onClose}>
      <div className="surface-card w-full max-w-md p-6 sm:p-8" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-start justify-between gap-3 mb-6">
          <div>
            <h3 className="text-display text-3xl text-foreground leading-none">{tr("Put it on your phone", "Instálala en tu móvil", "Installe-la sur ton téléphone")}</h3>
            <p className="mt-2 text-sm text-foreground/70">{tr("The fastest home-screen setup for PadelMatch.", "La forma más rápida de añadir PadelMatch a tu pantalla de inicio.", "La façon la plus rapide d'ajouter PadelMatch à ton écran d'accueil.")}</p>
          </div>
          <button onClick={onClose} className="p-2 rounded-full hover:bg-foreground/10 text-foreground/60" aria-label="Close">
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        {ios && (
          <div className="space-y-5">
            {!safari && (
              <div className="rounded-2xl border border-foreground/40 bg-foreground/10 p-4">
                <p className="text-sm font-bold text-foreground">{tr("First open this page in Safari", "Primero abre esta página en Safari", "Ouvre d'abord cette page dans Safari")}</p>
                <p className="mt-1 text-xs text-foreground/65">{tr("iPhone only shows “Add to Home Screen” inside Safari.", "En iPhone, “Añadir a la pantalla de inicio” solo aparece dentro de Safari.", "Sur iPhone, « Sur l'écran d'accueil » n'apparaît que dans Safari.")}</p>
                <div className="mt-3 flex gap-2">
                  ...
                  <button onClick={shareLink} className="inline-flex items-center justify-center rounded-full border border-foreground/25 px-4 py-2 text-foreground">
        ...
        {android && !deferredPrompt && (
          <div className="space-y-5">
            <div className="flex items-center gap-4">
              <div className="w-10 h-10 rounded-full bg-foreground/20 flex items-center justify-center shrink-0">
                <span className="text-display text-lg text-foreground">1</span>
              </div>
              <div>
                <p className="font-semibold text-foreground">{tr("Tap the menu (⋮)", "Toca el menú (⋮)", "Appuie sur le menu (⋮)")}</p>
                <p className="text-xs text-foreground/60">{tr("in Chrome's top-right corner", "en la esquina superior derecha de Chrome", "en haut à droite de Chrome")}</p>
              </div>
            </div>
            <div className="flex items-center gap-4">
              <div className="w-10 h-10 rounded-full bg-foreground/20 flex items-center justify-center shrink-0">
                <span className="text-display text-lg text-foreground">2</span>
              </div>
              <div>
                <p className="font-semibold text-foreground">{tr('Tap "Add to Home screen"', 'Toca "Añadir a la pantalla de inicio"')}</p>
                <p className="text-xs text-foreground/60">{tr("Then confirm", "Después confirma", "Puis confirme")}</p>
              </div>
            </div>
          </div>
        )}
        ...
        {!ios && !android && (
          <div className="text-center py-4">
            ...
            <div className="mb-5 flex justify-center gap-2">
              <button onClick={copyLink} className="inline-flex items-center gap-2 rounded-full border border-foreground/25 px-4 py-2 text-sm text-foreground hover:bg-[var(--ball)] hover:text-[var(--court-deep)] hover:border-[var(--ball)]">
                {copied ? <CheckCircle2 className="h-4 w-4" /> : <Copy className="h-4 w-4" />}
                {copied ? tr("Copied", "Copiado", "Copié") : tr("Copy link", "Copiar enlace", "Copier le lien")}
              </button>
              <button onClick={shareLink} className="inline-flex items-center gap-2 rounded-full border border-foreground/25 px-4 py-2 text-sm text-foreground hover:bg-foreground/10">
                <Send className="h-4 w-4" />
    ...
    <>
      {showMini && (
        <div className="fixed bottom-24 left-4 right-4 z-50 max-w-sm mx-auto">
          ...
          >
            <Smartphone className="w-6 h-6 text-foreground shrink-0" />
            <div className="flex-1 min-w-0">
              <p className="text-sm font-bold text-foreground">{tr("Put PadelMatch on your phone", "Instala PadelMatch en tu móvil", "Installe PadelMatch sur ton téléphone")}</p>
              <p className="text-xs text-foreground">{tr("Quick guided setup →", "Instalación guiada rápida →", "Installation guidée rapide →")}</p>
            </div>
            <Download className="w-5 h-5 text-foreground shrink-0" />
          </button>
        </div>
      )}
