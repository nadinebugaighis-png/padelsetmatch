...
const es: Dict = {
  ...
  "shell.back.grid": "Volver al Grid",
  "shell.grid": "Grid",
  "shell.admin": "Admin",
  "shell.core": "★ core",
  "shell.err.title": "Algo salió mal",
  ...
  "disc.world.on": "🌍 Mundo On",
  "disc.world.off": "🌍 Mundo Off",
  "disc.world.note": "Mostrando a todo el mundo (excepto quienes has ocultado). Desactívalo para volver a tu zona.",
...
export function LangSwitch({ className = "" }: { className?: string }) {
  ...
  const btn = (l: Lang, label: string) => (
    <button
      ...
      type="button"
      onClick={() => setLang(l)}
      className={`px-2.5 py-1 ${lang === l ? "bg-[var(--ball)] text-[var(--cream)] font-bold" : "text-[var(--ball)]/70 hover:text-[var(--ball)]"}`}
      aria-pressed={lang === l}
    >{label}</button>
  );
  return (
