...
function Discover() {
  ...
  // Quick variant used by the tiny thumbs-up on the card — silent connect, never navigates away.
              ...
              return (
                ...
                >
                  ...
                  <div className="relative bg-white p-2 shadow-[0_10px_28px_-12px_rgba(31,58,46,0.22)] rounded-[2px]">
                    <div className="relative aspect-[3/4] bg-[var(--paper-2)] overflow-hidden">
                      {c.photo_url && (
                        <img src={c.photo_url} alt={c.first_name} loading="lazy" decoding="async" className="absolute inset-0 w-full h-full object-cover transition-transform duration-500 group-hover:scale-[1.03]" />
                      )}
                      {!c.photo_url && (
                        <div className="absolute inset-0 flex items-center justify-center text-serif text-6xl text-[var(--ink)]/15">
                    ...
                    <div className="px-5 pt-4 pb-28 lg:px-6 lg:pt-6 lg:pb-24 space-y-4">
                      ...
                      {preview.free_court_access && (
                        <div className="flex items-center gap-2 text-[13px] text-[var(--ink)]">
                          <CourtIcon className="w-5 h-3 text-[var(--ink)] shrink-0" strokeWidth={2.2} />
                          <span className="font-medium">{tr("Free court access", "Pista gratis", "Terrain gratuit")}</span>
                          {preview.free_court_note && (
                            <span className="text-[var(--ink)]/60 truncate">· {preview.free_court_note}</span>
                          )}
                        </div>
                      )}
