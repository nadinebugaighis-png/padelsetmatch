...
function Matches() {
  ...
  return (
    <main className="px-4 py-5 max-w-md sm:max-w-2xl lg:max-w-4xl xl:max-w-5xl mx-auto">
      ...
      <p className="text-sm text-[var(--court-deep)]/70 mt-1">{t("ml.sub")}</p>

      {q.isLoading ? (
        <p className="mt-8 text-center text-[var(--court-deep)]/60">{t("ml.loading")}</p>
      ) : !q.data || q.data.length === 0 ? (
        <div className="mt-12 text-center text-[var(--court-deep)]/60">
          <p>{t("ml.empty")}</p>
        ...
        <ul className="mt-5 space-y-3">
          {q.data.map((m) => m.other && (
            <li key={m.match_id}>
              <Link to="/app/matches/$matchId" params={{ matchId: m.match_id }} className="flex items-center gap-3 surface-card p-3 hover:bg-[var(--court-deep)]/5 relative">
                <div className="w-16 h-16 rounded-xl overflow-hidden bg-[var(--court-deep)]/10 shrink-0 relative">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-[11px] uppercase tracking-widest text-[var(--court-deep)]/60">{m.other.zone} · {label(m.other.level)}</div>
                  <div className={`text-xs mt-1 truncate ${m.unread > 0 ? "text-[var(--court-deep)] font-medium" : "text-[var(--court-deep)]/70"}`}>
