...
function Matches() {
  ...
  return (
    <main className="programme-page px-4 py-5 min-h-[calc(100vh-4rem)]">
      <div className="max-w-md sm:max-w-2xl lg:max-w-4xl xl:max-w-5xl mx-auto">
        <h1 className="text-serif text-4xl text-[var(--ink)]">{t("ml.h1")}</h1>
        <p className="text-sm text-[var(--ink)]/70 mt-1">{t("ml.sub")}</p>
        ...
        {q.isLoading ? (
          ...
          <ul className="mt-5 space-y-3">
            {q.data.map((m) => m.other && (
              <li key={m.match_id}>
                <Link to="/app/matches/$matchId" params={{ matchId: m.match_id }} className="flex items-center gap-3 programme-card p-3 hover:bg-[var(--ink)]/[0.03] relative">
                  <div className="w-16 h-16 rounded-xl overflow-hidden bg-[var(--paper-2)] shrink-0 relative">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      <div className="text-serif text-2xl text-[var(--ink)]">{m.other.first_name}</div>
                    ...
                    <div className={`text-xs mt-1 truncate ${m.unread > 0 ? "text-[var(--ink)] font-medium" : "text-[var(--ink)]/70"}`}>
