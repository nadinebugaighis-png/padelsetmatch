...
function Matches() {
  ...
  return (
    <main className="px-4 py-5 max-w-md mx-auto">
      <h1 className="text-display text-4xl">Your courts</h1>
      <p className="text-sm text-[var(--cream)]/70 mt-1">Everyone here tapped you back.</p>

      {q.isLoading ? (
        <p className="mt-8 text-center text-[var(--cream)]/60">Loading…</p>
      ) : !q.data || q.data.length === 0 ? (
        <div className="mt-12 text-center text-[var(--cream)]/60">
          <p>No matches yet.</p>
          <Link to="/app" className="underline mt-2 inline-block">Discover players →</Link>
        </div>
      ) : (
            ...
            <li key={m.match_id}>
              <Link to="/app/matches/$matchId" params={{ matchId: m.match_id }} className="flex items-center gap-3 surface-card p-3 hover:bg-[var(--cream)]/5">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-[11px] uppercase tracking-widest text-[var(--cream)]/60">{m.other.zone} · {m.other.level}</div>
