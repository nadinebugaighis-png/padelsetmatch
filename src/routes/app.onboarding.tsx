...
function Onboarding() {
  ...
  if (profileQ.isLoading) {
    return (
      <main className="px-4 py-10 max-w-md sm:max-w-2xl lg:max-w-4xl xl:max-w-5xl mx-auto text-center text-[var(--court-deep)]/70">
  ...
  if (profileQ.isError) {
    return (
      <main className="px-4 py-10 max-w-md sm:max-w-2xl lg:max-w-4xl xl:max-w-5xl mx-auto text-center space-y-4">
        <p className="text-[var(--court-deep)]/80">
  ...
  return (
    <main className="px-4 py-6 max-w-md sm:max-w-2xl lg:max-w-4xl xl:max-w-5xl mx-auto">
      <div className="flex items-center justify-between text-xs uppercase tracking-widest text-[var(--court-deep)]/60">
        <span>{t("ob.step")} {step + 1} {t("ob.of")} {steps.length}</span>
        <span>{steps[step]}</span>
      </div>
      <div className="h-1 mt-2 rounded-full bg-[var(--court-deep)]/5 overflow-hidden">
      ...
      <div className="mt-6 surface-card p-5 space-y-4">
        {step === 0 && (
          <>
            ...
            <label className="text-xs uppercase tracking-widest text-[var(--court-deep)]/60">{t("ob.firstName")}</label>
            <Input value={first_name} onChange={(e) => setFirstName(e.target.value)} placeholder={t("ob.firstNamePh")} />
            <label className="text-xs uppercase tracking-widest text-[var(--court-deep)]/60">{t("ob.age")}</label>
            <AgeInput value={age} onCommit={setAge} placeholder={tr("e.g. 32", "p. ej. 32", "p. ex. 32")} />
            <p className="text-[11px] text-[var(--court-deep)]/50">{tr("Enter your age (18–99), not your birth year.", "Introduce tu edad (18–99), no tu año de nacimiento.", "Saisis ton âge (18–99), pas ton année de naissance.")}</p>
            <label className="text-xs uppercase tracking-widest text-[var(--court-deep)]/60">{t("ob.iAm")}</label>
            ...
            <label className="text-xs uppercase tracking-widest text-[var(--court-deep)]/60">{tr("What are you looking for?", "¿Qué estás buscando?", "Que cherches-tu ?")}</label>
            <div className="flex flex-wrap gap-2">
              {[
            ...
            <p className="text-xs text-[var(--court-deep)]/50">{t("ob.privateNote")}</p>
          </>
        )}
        {step === 1 && (
          <>
            ...
            {hasPartnerGoal && (
              <>
                <label className="text-xs uppercase tracking-widest text-[var(--court-deep)]/60">{tr("Who would you like to meet?", "¿A quién te gustaría conocer?", "Qui veux-tu rencontrer ?")}</label>
                <div className="flex flex-wrap gap-2">
            ...
            {hasPartnerGoal && meetPref === "everyone" && (
              <div className="rounded-lg border border-[var(--court-deep)]/10 p-3 space-y-2">
                <div className="text-xs uppercase tracking-widest text-[var(--court-deep)]/60">{tr("Advanced profile (optional)", "Perfil avanzado (opcional)", "Profil avancé (optionnel)")}</div>
                <label className="text-[11px] text-[var(--court-deep)]/60">{tr("Sexual orientation", "Orientación sexual", "Orientation sexuelle")}</label>
                <Input
                  ...
                  maxLength={60}
                />
                <p className="text-[10px] text-[var(--court-deep)]/50">{tr("Private — used only to improve matches. Not shown on your profile.", "Privado — solo se usa para mejorar tus matches. No aparece en tu perfil.", "Privé — utilisé seulement pour améliorer les matches. Pas affiché sur ton profil.")}</p>
              </div>
            )}

            <label className="text-xs uppercase tracking-widest text-[var(--court-deep)]/60">{t("ob.ageRange")}</label>
          ...
          <>
            ...
            <div className="space-y-3">
              {locBlocks.map((b, i) => {
                ...
                return (
                  <div key={i} className="rounded-lg border border-[var(--court-deep)]/15 p-3 space-y-2">
                    <div className="flex items-center justify-between">
                      <span className="text-xs uppercase tracking-widest text-[var(--court-deep)]/60">{tr("Location", "Ubicación", "Lieu")} {i + 1}</span>
                      {locBlocks.length > 1 && (
                        <button type="button" onClick={() => removeBlock(i)} className="text-[var(--court-deep)]/60 hover:text-[var(--clay)]">
                          <X className="w-4 h-4" />
                        </button>
                      )}
                    </div>
                    <select
                      ...
                      className="w-full bg-transparent border border-[var(--court-deep)]/20 rounded-md h-9 px-2 text-sm text-[var(--court-deep)]"
                    >
                    ...
                    {cities.length > 0 ? (
                      <select
                        ...
                        className="w-full bg-transparent border border-[var(--court-deep)]/20 rounded-md h-9 px-2 text-sm text-[var(--court-deep)] disabled:opacity-50"
                      >
                        ...
                        {b.areas.map((a, ai) => {
                          ...
                          return (
                            <div key={ai} className="flex gap-2">
                              ...
                              />
                              {a && (
                                <button type="button" onClick={() => updateArea(i, ai, "")} className="text-[var(--court-deep)]/60 hover:text-[var(--clay)] px-2">
                                  <X className="w-4 h-4" />
                                </button>
                              )}
                            </div>
                    ...
                    {isReal(b.country) && !isReal(b.city) && (
                      <p className="text-[11px] text-[var(--court-deep)]/80">
            ...
            <label className="text-xs uppercase tracking-widest text-[var(--court-deep)]/60">{t("ob.nat")}</label>
            <select className="w-full bg-transparent border border-[var(--court-deep)]/20 rounded-md h-9 px-2" value={nationality} onChange={(e) => setNationality(e.target.value)}>
            ...
            <label className="text-xs uppercase tracking-widest text-[var(--court-deep)]/60">{t("ob.langs")}</label>
            <div className="flex flex-wrap gap-2">
              {LANGUAGES.map((l) => (
            ...
            <label className="text-xs uppercase tracking-widest text-[var(--court-deep)]/60">{tr("Your padel style (pick up to 3)", "Tu estilo de pádel (elige hasta 3)", "Ton style de padel (jusqu'à 3)")}</label>
            <div className="flex flex-wrap gap-2">
              {PADEL_STYLES.map((s) => {
            ...
            <label className="text-xs uppercase tracking-widest text-[var(--court-deep)]/60">{t("ob.padelLevel")}</label>
            <div className="flex flex-wrap gap-2">
              {PADEL_LEVELS.map((l) => (
                <button key={l} onClick={() => setLevel(l)} className={`chip ${level === l ? "chip-ball" : ""}`}>{label(l)}</button>
              ))}
            </div>
            <label className="text-xs uppercase tracking-widest text-[var(--court-deep)]/60">{tr("Preferred court side", "Lado de pista preferido", "Côté de pista préféré")}</label>
            <div className="flex flex-wrap gap-2">
              {COURT_SIDES.map((s) => (
            ...
            <label className="text-xs uppercase tracking-widest text-[var(--court-deep)]/60">{tr("When can you play?", "¿Cuándo puedes jugar?", "Quand peux-tu jouer ?")}</label>
            <div className="flex flex-wrap gap-2">
            ...
            <div className="rounded-xl border border-[var(--court-deep)]/30 bg-[var(--cream)]/5 p-3 mt-2">
            ...
            <label className="text-xs uppercase tracking-widest text-[var(--court-deep)]/60">{t("ob.bio")}</label>
        ...
        {step === 3 && (
          <>
            ...
            <p className="text-sm text-[var(--court-deep)]/70">{t("ob.h3sub")} <b>{t("ob.h3priv")}</b></p>

            <label className="text-xs uppercase tracking-widest text-[var(--court-deep)]/60">{t("ob.suggested")}</label>
            <div className="flex flex-wrap gap-2">
            ...
            <label className="text-xs uppercase tracking-widest text-[var(--court-deep)]/60">{t("ob.addOwn")}</label>
            ...
            {priorities.length > 0 && (
              <>
                <label className="text-xs uppercase tracking-widest text-[var(--court-deep)]/60">{t("ob.ranking")}</label>
                <ul className="space-y-2">
                  {priorities.map((tr, i) => (
                    <li key={tr} className="flex items-center gap-2 bg-[var(--cream)]/5 rounded-md px-3 py-2">
                      <span className="text-[var(--court-deep)] font-bold w-6">{i + 1}.</span>
                      ...
                      <button onClick={() => removePriority(tr)} className="p-1 text-[var(--court-deep)]/60 hover:text-[var(--clay)]"><X className="w-4 h-4" /></button>
                    </li>
                  ))}
                </ul>
                <p className="text-xs text-[var(--court-deep)]/50">{t("ob.pickThree")}</p>
              </>
            )}

            <label className="text-xs uppercase tracking-widest text-[var(--court-deep)]/60">{tr("Personal characteristics (pick up to 10)", "Características personales (elige hasta 10)", "Traits personnels (jusqu'à 10)")}</label>
            <div className="flex flex-wrap gap-2">
            ...
            <p className="text-[11px] text-[var(--court-deep)]/50">{personalTraits.length}/10 {tr("selected", "seleccionados", "sélectionnés")}</p>
        ...
        {step === 4 && (
          <>
            ...
            <p className="text-sm text-[var(--court-deep)]/70">{t("ob.h4sub")}</p>
            <label className="block aspect-[3/4] rounded-2xl border border-dashed border-[var(--court-deep)]/30 overflow-hidden relative cursor-pointer">
              {photoUrl ? (
                <>
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center text-[var(--court-deep)]/60 gap-2">
                  <Camera className="w-8 h-8" />
                  <span className="text-sm">{uploading ? t("ob.uploading") : t("ob.tapUpload")}</span>
                  <span className="text-[11px] text-[var(--court-deep)]/50 px-6 text-center">{tr("Tip: a photo with your racket gets 3× more matches 🎾", "Consejo: una foto con tu pala consigue 3× más matches 🎾", "Astuce : une photo avec ta raquette obtient 3× plus de matches 🎾")}</span>
                </div>
              )}
              <input type="file" accept="image/*" className="hidden" onChange={(e) => { const f = e.target.files?.[0]; if (f) uploadPhoto(f); }} />
            </label>
            {!photoUrl && (
              <p className="text-[11px] text-[var(--court-deep)]/55 text-center">
