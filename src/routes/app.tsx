...
function AuthShell() {
  ...
  return (
    <div className="min-h-screen pb-28 programme-page">
...
function NavTab({ to, label, ariaLabel, icon, active, highlight, badge }: { to: string; label: string; ariaLabel?: string; icon: React.ReactNode; active: boolean; highlight?: boolean; badge?: number }) {
  ...
  return (
    <Link to={to} aria-label={ariaLabel} className={`flex min-h-[76px] flex-col items-center justify-center gap-2 px-1 text-[11px] font-bold uppercase tracking-[0.08em] relative ${active ? "text-[var(--cream)]" : isHighlight ? "text-[var(--plum)]" : "text-[var(--cream)]/55"}`}>
