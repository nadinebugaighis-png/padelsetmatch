    ...
    <main className="min-h-screen flex flex-col">
      <header className="px-6 py-5 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <LangSwitch />
          <Link to="/auth" search={{}} className="chip chip-ball">{t("land.signin")}</Link>
        </div>
      </header>
      ...
      <section className="flex-1 grid lg:grid-cols-2 gap-10 items-center px-6 lg:px-16 py-10">
        <div className="max-w-xl">
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            <Link to="/auth" search={{}} className="inline-flex items-center rounded-full bg-[var(--ball)] text-[var(--court-deep)] font-semibold px-6 py-3 hover:opacity-90">
              {t("land.cta")}
            </Link>
      ...
      <section className="px-6 lg:px-16 py-12 border-t border-[var(--cream)]/10">
        <div className="flex items-end justify-between mb-6 flex-wrap gap-3">
          ...
          <Link to="/auth" search={{}} className="inline-flex items-center rounded-full bg-[var(--ball)] text-[var(--court-deep)] font-semibold px-5 py-2.5 hover:opacity-90">
            {t("land.preview.cta")}
          </Link>
        </div>
          ...
          {[
            ...
            <div key={i} className="relative aspect-[3/4] rounded-2xl overflow-hidden border border-[var(--cream)]/10 group">
              ...
              <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition bg-black/40">
                <Link to="/auth" search={{}} className="rounded-full bg-[var(--ball)] text-[var(--court-deep)] font-semibold px-4 py-2 text-sm">{t("land.preview.unlock")}</Link>
              </div>
            </div>
          ))}
        </div>
